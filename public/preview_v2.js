(() => {
  /** badminton_v2 / display_v2.json：仅赛事预告三场 + images（键顺序即组内展示顺序） */
  ...
  function appendCopyFieldRow(parent, k, obj, copyHints) {
    ...
    if (hintStr) {
      ...
      placeholder.textContent =
        "（暂无说明 · 可由开发在 config/display-copy-labels-v2.json 补充）";
  ...
  function renderCopyGrouped(root, obj, copyHints) {
    ...
    if (!keys.length) {
      ...
      p.textContent = "（暂无 copy 键 · 请在 config/display_v2.json 补充）";
      root.appendChild(p);
      return;
    }
  ...
  function renderImagesSection(section, root, obj, hints) {
    ...
    if (!keys.length) {
      const p = document.createElement("p");
      p.className = "preview-empty muted";
      p.textContent = "（暂无 images 键 · 请在 config/display_v2.json 补充）";
      root.appendChild(p);
      return;
    }
  ...
  async function saveSection(section) {
    ...
    try {
      const res = await fetch("/api/display-v2/patch", {
        method: "POST",
      ...
      setStatus("live", "已保存 · 等待 SSE 同步（/events/v2）");
    } catch (e) {
  ...
  const es = new EventSource("/events/v2");
